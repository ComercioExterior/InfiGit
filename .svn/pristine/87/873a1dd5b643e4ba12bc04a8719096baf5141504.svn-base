...
/**
 * Clase que recupera los datos de la pagina de actualizacion de las relaciones entre Unidad de Inversion e Indicadores y aplica la persistencia en la base de datos
 * @author Megasoft Computaci&oacute;n
 */
public class UIIndicadoresUpdate extends AbstractModel implements UnidadInversionConstantes {
	...
	public void execute() throws Exception {	
		...
		String [] strIdIndicadorAnterior = _req.getParameterValues("idIndicadorAnterior");
		String [] strIndicaRequeridoAnterior = _req.getParameterValues("indicaRequeridoAnterior");	
		...
		for (int i = 0; i < ca; i++) {
			...
			idsIndicadorPrevio[i] = strIdIndicadorAnterior[i];
			indicaRequeridoPrevio[i] = new Integer(strIndicaRequeridoAnterior[i]).intValue();
		}
