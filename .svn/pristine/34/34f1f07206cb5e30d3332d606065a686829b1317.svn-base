 ...
 */  
public class CalculadorDeTomaDeOrdenesDivisasPersonas extends AbstractModel{
	...
	public void execute() throws Exception {
		...
		_datos.append("nro_solicitud", java.sql.Types.VARCHAR);
		_datos.addNew();
		
		_datos.setValue("nro_solicitud", nro_solicitud);
	...
	public boolean isValid() throws Exception {
		...
		if (flag)
		{
			//System.out.println("ORDEN ID---: "+_req.getParameter("orden_id"));
			...
			if(datos_unidad_inv.count()>0){
				...
				uiBlotterRangos = uiBlotterRangosDAO.listarBlotterRangosObj(idUnidadInversion, idBlotter, cliente.getTipoPersona());
							System.out.println("bloter rangos "+uiBlotterRangos);
								...
								BigDecimal tasaPropuesta = new BigDecimal(0) ;
									...
									if(datos_unidad_inv.getValue("undinv_in_pedido_monto")!=null && datos_unidad_inv.getValue("undinv_in_pedido_monto").equals("1")){
										...
										if(_record.getValue("tasa_propuesta")!=null){							
											...
											tasaPropuesta = new BigDecimal(_record.getValue("tasa_propuesta"));										
