...
public class RecepcionCruceOperacionesDICOM extends BatchOps implements Runnable{
	
	public RecepcionCruceOperacionesDICOM(DataSet unidadesInvProcesar,String usuarioGenerico,DataSource datasource) throws Exception {
		super();		
		_dso = datasource;
	...
	public RecepcionCruceOperacionesDICOM(DataSource datasource) throws Exception {
		super();		
		_dso = datasource;
		controlArchivoDAO= new ControlArchivoDAO(_dso);
	...
	public void run() {
		...
		try {		
			...
			//TRAN = TransaccionNegocio.CICLO_BATCH_DICOM+monedaSubasta;
			
				TRAN = getTransaccionNegocio();
			
			//VERIFICAR CICLO
			System.out.println("TRAN--->"+TRAN);
			if (verificarCiclo(TRAN) ){
				...
				//tipomoneda= parametrosRecepcionDICOM.get(ParametrosSistema.UNDINV_MONEDA_DEMANDA);
				...
				/*if(Utilitario.longitudValida(jornadaDicom, 1, EstructuraArchivoOperacionesDICOM.NRO_JORNADA.getValor())&&
					...
					//if(Utilitario.betweenDates(fechaInicioDate, fechaFinDate, fechaActual)){	
						...
						//REGISTRAR PROCESO
						//transaccionEjecutar=TransaccionNegocio.PROC_CRUCE_DICOM_INTERBANCARIO+parametrosRecepcionDICOM.get(monedaSubasta);//nm36635 REVISAR
							...
							try { 
								...
								try {
									if(jornadaManual.equals("0")){
										...
										System.out.println("rutaRecepcion--->"+rutaRecepcion);
										logger.info("CruceOperacionesDICOM-> Antes de Consumir el servicio bajarOrdenDesbloqueosCreditosDebitos. Jornada: "+jornadaDicom);
										
										OperCamFileTransferPortBindingStub stub = new OperCamFileTransferPortBindingStub(null,_dso);
										 byte[] _bytes;						
										_bytes=stub.bajarOrdenDesbloqueosCreditosDebitos(jornadaDicom, monedaSubasta);
										
										logger.info("CruceOperacionesDICOM-> Despues de Consumir el servicio bajarOrdenDesbloqueosCreditosDebitos");
										
										//SE ESCRIBE EL ARCHIVO PROVISIONALMENTE EN EL SERVIDOR 										
										Utilitario.byteToFile(_bytes,rutaRecepcion);
										//logger.error("CruceOperacionesDICOM-> Se escribe Archivo"+Utilitario.openFileToString(_bytes)+ " Ruta" + rutaRecepcion);
									}else{
										rutaRecepcion=rutaRecepcion+getNombreArchivoRecepcionManualRespaldo(jornadaDicom);
										System.out.println("rutaRecepcionmanual--->"+rutaRecepcion);
										logger.info("CruceOperacionesDICOM-> Prueba de Consumo Manual de archivo de operaciones Preaprobadas" + " Ruta" + rutaRecepcion);									
									}									
									
									//LEER ARCHIVO
									archivoRecibido=getArchivo(rutaRecepcion);
									
									transferirArchivo(rutaRecepcion, getDestinoFinal());//MODO CONTINGENCIA QUITAR AL FINALIZAR DESARROLLO DE CRUCE
									logger.info("CruceOperacionesDICOM-> Después de Transferir Archivo en Cruce Ruta: " + rutaRecepcion);
									//PROCESAR ARCHIVO
									//recepcionArchivoOpCruzadas(archivoRecibido,jornadaDicom);	
									
									//RESPALDAR ARCHIVO RECIBIDO
									respaldarArchivo(archivoRecibido);
									logger.info("CruceOperacionesDICOM-> Después de Respaldar Archivo en Cruce Ruta: " + rutaRecepcion);
								} catch (Exception e1) {
									if(e1.getMessage().equals(DESCUADRE_TOTALES_EXCEPTION)||e1.getMessage().startsWith(ERROR_CABECERA_EXCEPTION)){
										...
										proceso.agregarDescripcionErrorTrunc("CruceOperacionesDICOM->Error: "+e1.getMessage(),true);
									}
									
									logger.error("CruceOperacionesDICOM-> "+e1.getMessage());
									e1.printStackTrace();
								}
	
							} catch (Exception e) {
								proceso.agregarDescripcionErrorTrunc("CruceOperacionesDICOM->Error: "+e.getMessage(),true);
								logger.error("CruceOperacionesDICOM-> "+e.getMessage());
								e.printStackTrace();
								
							}
						}else{
							logger.info("CruceOperacionesDICOM-> Ya existe un proceso en ejecucion o ha ocurrido un error al intentar registrar el proceso");
						}
				...
				logger.info("CruceOperacionesDICOM-> Ya se encuentra registrado un ciclo en ejecución");
			}
		} catch (Exception ex) {
			logger.error("Error en el proceso de recepción archivo batch para CruceOperacionesDICOM. ", ex);		
			...
			terminarProceso();
			logger.info("Terminado el proceso de recepción de archivo batch para CruceOperacionesDICOM... ");
		}
	}
	...
	protected void procesarArchivo(File archivo, String jornadaDicom) throws Exception {
		...
		SolicitudDICOM registroCruceDicom;
		...
		//PROCESAR CABECERA //TODO VERIFICAR SI SE PROCESA ALGUNA CABECERA
//		if (linea != null) {
//			if (linea.length() > 0) {		
//				//SE CARGAN LOS TOTALES QUE SERAN VALIDADOS UNA VEZ SE TERMINEN DE REGISTRAR EN BD
//				formatoArchivoRecepcion.cargarCabeceraPreaprobadas(linea);//TODO VERIFICAR SI EL ARCHIVO CONTIENE CABECERA
//				cabecera=linea;
...
//		}
		...
		configurarCodigosOperacion(this.unidadesInvProcesar,jornadaDicom);
		linea = archivoRecepcion.leerLinea();
		...
		while (linea != null) {
				if (linea.length() > 0) {					
					try {
						...
						formatoArchivoRecepcion.cargarCuerpoOpCruzadas(linea);
						
						//VALIDAR CUERPO OPERACIONES PREAPROBADAS
						if(((formatoArchivoRecepcion.getCorrelativo().trim()).length()>0)){
							registroCruceDicom=formatoArchivoRecepcion.validarCuerpoOpCruzadas();							
							if(registroCruceDicom!=null){
								//ejecutarCommit=procesarQuerysPorLote();
								ejecutarCommit=true;
								registroCruceDicom.setIdJornada(jornadaDicom);
								
								procesarAdjudicacion(registroCruceDicom);
								//solicitudesDICOMDAO.registrarSolicitudDICOM(solicitudDICOM,ejecutarCommit);
							  	//PROCESO DE DETERMINACION DE ADJUDICACION 
	...
	protected String getTransaccionNegocio(){
		...
		if(monedaInt==constatemoneda){
			...
			transsaciones =TransaccionNegocio.CICLO_BATCH_DICOM+parametrosRecepcionDICOM.get(monedaSubasta);
		}
	...
	protected String getTransaccionEjecutar(){
		String TransaccionEjecutar="";
		if(monedaInt==constatemoneda){
			TransaccionEjecutar = TransaccionNegocio.PROC_CRUCE_DICOM_INTERBANCARIO;
		}else{
			TransaccionEjecutar =TransaccionNegocio.PROC_CRUCE_DICOM_INTERBANCARIO+parametrosRecepcionDICOM.get(monedaSubasta);
		}
	...
	//	return "cruce_recepcion_"+jornada+".txt";
		...
		String nombreo="";
		if(monedaInt==constatemoneda){
		nombreo="cruce_recepcion_"+jornada+".txt";
		}else{
		nombreo="cruce_recepcion_"+jornada+parametrosRecepcionDICOM.get(monedaSubasta)+".txt";
		}
		return nombreo;
	}
	...
	protected String getNombreArchivoRecepcionManualRespaldo(String jornada) throws ParseException{		
		...
		String nombreo="";
		if(monedaInt==constatemoneda){
			nombreo="cruzadas_recepcion_manual_"+jornada+".txt";
		}else{
			nombreo="cruzadas_recepcion_manual_"+jornada+parametrosRecepcionDICOM.get(monedaSubasta)+".txt";
		}
	...
	protected String getNombreArchivoEnvio(String jornada) throws ParseException{		
		...
		String nombreo="";
		if(monedaInt==constatemoneda){
			nombreo="preaprobadas_envio_"+jornada+".txt";
		}else{
			nombreo="preaprobadas_envio_"+jornada+parametrosRecepcionDICOM.get(monedaSubasta)+".txt";
		}
	...
	protected String getDestinoFinal(){
		...
		String destino1="";
		if(monedaInt==constatemoneda){
			destino.replaceAll(ConstantesGenerales.CODIGO_DOLAR,monedaSubasta);
			destino1=destino;
		 //destino=parametrosRecepcionDICOM.get(getNombreArchivo()+ConstantesGenerales.CODIGO_DOLAR);
		}else{
			//destino.replaceAll(ConstantesGenerales.CODIGO_DOLAR,monedaSubasta);
		destino1=destino.replaceAll(ConstantesGenerales.CODIGO_DOLAR,parametrosRecepcionDICOM.get(monedaSubasta));
		}
		System.out.println("destino--->"+destino);
		return destino1;
		//return parametrosRecepcionDICOM.get(getNombreArchivo());
	}
	//REVISAR CON FRANCISCO 
	...
	private void procesarAdjudicacion(SolicitudDICOM registroCruce)throws Exception{
		...
		if(solicitudDICOM!=null){	
			...
			if(solicitudDICOM.getTipoOperacion().equals(ConstantesGenerales.SOLICITUD_OFERTA)){//SI la operaciones es OFERTA se compara el campo MONTO_OP_NACIONAL con el registro de Credito Bs del archivo de cruce
				if(registroCruce.getMontoCreditoBs().equals(new BigDecimal(0))){
				   //Solicitud OFERTA NO ADJUDICADA
					solicitudDICOM.setMontoOperacionVEF(new BigDecimal(0));
					solicitudDICOM.setMontoOperacionUSD(new BigDecimal(0));   	
				} else if(!(solicitudDICOM.getMontoOperacionVEF().equals(registroCruce.getMontoCreditoBs()))){//Monto Adjudicado menor al solicitado
				...
				if(registroCruce.getMontoCreditoDiv().equals(new BigDecimal(0))){
					//Solicitud DEMANDA NO ADJUDICADA
					solicitudDICOM.setMontoOperacionVEF(new BigDecimal(0));
					solicitudDICOM.setMontoOperacionUSD(new BigDecimal(0));
				} if(!(solicitudDICOM.getMontoOperacionUSD().equals(registroCruce.getMontoCreditoDiv()))){//Monto Adjudicado menor al solicitado
					adjudicacionParcial=true;
