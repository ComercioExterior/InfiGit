package models.consultas_seguridad.consulta_opciones_url;
...
public class Table extends MSCModelExtend
{
	...
	public void execute() throws Exception
	{
		...
		_table.append("ruta",java.sql.Types.VARCHAR);
		...
		if(_record.getValue("id_application")!=null && _req.getParameter("band")!=null){
			...
			_record.setValue("cab_applic", "<th width='8%'> Aplic. </th>");
		}
		...
		while(_temp.next()){
			...
			//ruta de la opcion principal es ella misma.
			//_table.setValue("ruta", _temp.getValue("st_nombre"));
			...
			if(Integer.parseInt(_temp.getValue("nu_hijos"))>0){
				...
				this.buscar_hijos(_temp.getValue("id"), _table.getValue("columna_applic"), _temp.getValue("st_nombre"));
			}
		}
	 ...
	 */
	public void buscar_hijos(String id_padre, String aplicacion, String nombre_padre) throws Exception{
		...
		while(_aux.next()){	
			...
			_table.setValue("columna_applic", aplicacion);			
			
			/*if(_table.getValue("ruta")==null || _table.getValue("ruta").equals("null"))
				_table.setValue("ruta", ".");
			
			_table.setValue("ruta", _table.getValue("ruta")+ "/" + nombre_padre );*/
			...
			if(Integer.parseInt(_aux.getValue("nu_hijos"))>0){
				...
				buscar_hijos(_aux.getValue("id"), aplicacion, nombre_padre);
			}
		}
