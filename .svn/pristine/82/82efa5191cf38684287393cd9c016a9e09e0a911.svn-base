	...
	protected void getOperaciones() throws Exception{
		...
		if(seleccion.equalsIgnoreCase("todos")){
			...
			if(tipoFiltro.equals("EXCLUIR")){
				...
				DataSet listaTodos =  new DataSet();
				listaTodos = getSessionDataSet("listarUnidadesParaCobroAdjBatchSubastaDivisasPersonal");
				boolean flag = true;
				while(listaTodos.next()){
					//TODO  verificar el nombre del elemento en el dataset
					//String idOperacion = listaTodos.getValue("ordene_operacion_id");
		...
		operaciones = statement.executeQuery(inversionDAO.previoDeOperacionBatchPorIdOrden(ordenesIdEspecificos,_req.getParameter("undinv_id"),Transaccion,StatusOrden.ADJUDICADA,StatusOrden.REGISTRADA,StatusOrden.NO_ADJUDICADA_INFI,StatusOrden.CRUZADA,StatusOrden.NO_CRUZADA));
	}
