...
/**
 * Clase encargada insertar en Base de Datos una Transacci&oacute;n Financiera.
 * @author Erika Valerio, Megasoft Computaci&oacute;n
 */
public class UIComisionInsert extends AbstractModel implements UnidadInversionConstantes
{		
	String[] reglas;
	/**
	...
	{				
			...
			//idUnidadInversion = Integer.parseInt(unidadInversion);
				...
				String idComisionUI = GenericoDAO.dbGetSequence(_dso, ConstantesGenerales.SECUENCIA_UI_COMISIONES);
				...
				if(reglas!=null){
					...
					for(int i=0; i< reglas.length; i++){
						//guardar los clientes asociados a una regla de transacci&oacute;n financiera en un arreglo
						double monto = 0;
						double porcentaje = 0;
				...
				//insertar Transaccion Financiera
				arrSql = uiComisionDAO.insertar(uiComision);
	...
	{	uIComisionDAO= new UIComisionDAO(_dso);
		...
		{				
			...
			//Validar si ya existen comisiones registradas para un tipo de operacion (efectivo, electrónico)
			int cant=uIComisionDAO.cantidadComisionesUIporTipo(Integer.toString((idUnidadInversion)),Integer.parseInt(_record.getValue("trnfin_id_op")));
			if(cant>0){
				_record.addError("Unidad de Inversi&oacute;n / Comisiones", "Ya existe una comisión registrada para este Tipo de Operación");
				error = true;
			}
