...
public class FormatoOperacionesDICOMInterbancario {
	...
	public SolicitudDICOM validarCuerpoOpCruzadas() throws ParseException {
		SolicitudDICOM solicitudDICOM=null;
		BigDecimal monto=null;
		Date fecha=null;
		boolean datosOK=true;
		...
		try {
			if(((this.correlativo.trim()).length()!=0)){
				solicitudDICOM=new SolicitudDICOM();
				solicitudDICOM.setIdOperacion(correlativo);			
			}else{
				datosOK=false;
			}
			//Operaciones de DESBLOQUEO
			if(this.montoDesloqueoBs.trim().length()!=0){
				solicitudDICOM.setMontoDesbloqueoBs(new BigDecimal(montoDesloqueoBs));
			}
			
			if(this.montoCreditoDivisa.trim().length()!=0){
				solicitudDICOM.setMontoDesbloqueoDiv(new BigDecimal(montoCreditoDivisa));
			}
			
			if(this.codigoDivisaCredito.trim().length()!=0){
				solicitudDICOM.setMonedaDesbloqueoDiv(codigoDivisaDesloqueo);
			}
			
			//Operaciones de CREDITO
			if(this.montoCreditoBs.trim().length()!=0){
				solicitudDICOM.setMontoCreditoBs(new BigDecimal(montoCreditoBs));
			}
			
			if(this.montoCreditoDivisa.trim().length()!=0){
				solicitudDICOM.setMontoCreditoDiv(new BigDecimal(montoCreditoDivisa));
			}
			
			if(this.codigoDivisaCredito.trim().length()!=0){
				solicitudDICOM.setMonedaCreditoDiv(codigoDivisaCredito);
			}
			//Operaciones de DEBITO
			if(this.montoDebitoBs.trim().length()!=0){
				solicitudDICOM.setMontoDebitoBs(new BigDecimal(montoDebitoBs));
			}
			
			if(this.montoDebitoDivisa.trim().length()!=0){
				solicitudDICOM.setMontoDebitoDiv(new BigDecimal(montoDebitoDivisa));
			}
			
			if(this.codigoDivisaDebito.trim().length()!=0){
				solicitudDICOM.setMonedaDebitoDiv(codigoDivisaDebito);
			}
