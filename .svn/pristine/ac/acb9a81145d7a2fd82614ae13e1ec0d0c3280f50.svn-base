...
public class Generar extends MSCModelExtend {
	...
	public void execute() throws Exception {
		...
		if(seleccion.equalsIgnoreCase("todos")){			
			...
			if(tipoFiltro.equals("EXCLUIR")){
				...
				listaTodos = getSessionDataSet("listarUnidadesParaCobroAdjBatchDicom");
				boolean flag = true;
		...
		/***************/
		int transaccionFinanciera=0;//Tipo de transaccion que define si es capital o comision (cero es ambas)
		...
		consultaSolicitudes=archivosOpsDAO.envioOperacionesDICOM(idOrdenes,idUniInv,TransaccionNegocio.TOMA_DE_ORDEN,StatusOrden.ENVIADA,transaccionFinanciera,trnfTipo);
	...
	public boolean isValid() throws Exception {
		...
		if (procesosDAO.getDataSet().count() > 0) {
			_record
					.addError(
							"Dicom Cobro Deman Batch Subasta",
