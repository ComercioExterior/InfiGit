...
public class Browse extends MSCModelExtend {
	...
	public boolean isValid() throws Exception {
		...
		if (flag){
			    //Se verifica si no existen órdenes pendientes para la unidad de inversión seleccionada
				UnidadInversionDAO inversionDAO = new UnidadInversionDAO(_dso);
				...
				if (dataSet.count()>0){
					_record
					.addError(
							"Unidad de Inversi&oacute;n", "Existen órdenes para la unidad de inversión que no han pasado por el proceso de adjudicación. " +
									"Por favor verifique e intente más tarde");
							flag = false;					
				}
		}
		return flag;
