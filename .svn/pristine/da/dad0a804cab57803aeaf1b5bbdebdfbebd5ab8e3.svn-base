...
function exportarCVS() {
	llenarVaribalesHtml();
		...
		if (confirm("Se Generará un archivo CSV con todas las operaciones consultadas. Esto puede tardar varios minutos. ¿Está seguro que desea iniciar el proceso?")){
			...
			window.location="intercambio_mesas_cambios-export?framework.controller.outputstream.open=false&statusp="+tipoBusqueda+"&fecha="+fecha+"&statusE="+statusEnvio+"&Tipo="+tipoOperacion;
			return true;
		}else{
...
////	alert("detalles");
//	llenarVaribalesHtml();
//	// ruta es la accion a ejecutar el metodo, tambien añadimos las variables a pasar al siguiente java
