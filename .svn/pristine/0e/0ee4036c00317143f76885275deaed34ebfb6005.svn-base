package models.picklist.pick_clientes;
 ...
 */
public class PickCliente extends AbstractModel
{
	/*** Logger APACHE*/
	private Logger logger = Logger.getLogger(PickCliente.class);
				...
				if (clienteDAO.getDataSet().count()==0){
					...
					if (cedRif > 0){
						try {
							...
							clienteNuevo.setNumeroPersona(clienteWS.getNumeroPersona().trim());
