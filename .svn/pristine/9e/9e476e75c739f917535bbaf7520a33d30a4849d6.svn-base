...
public class DetalleVentaTitulo extends ExportableOutputStream{


	Logger logger = Logger.getLogger(DetalleVentaTitulo.class);
	/**
			...
			while (operaciones.next()) {			
				...
				/*try {
					...
					while (operacionesAsociadas.next()) {						
						escribir("\r\n"+separador+separador+separador+separador+separador+separador+separador+separador+separador);
	...
	protected void getOperaciones() throws Exception{		
		operaciones = statement.executeQuery(ordenDAO.listarDetalleOrdenPago(_req.getParameter("client_id"),_req.getParameter("titulo_id"),_req.getParameter("fe_transac_desde"),_req.getParameter("fe_transac_hasta"),_req.getParameter("status_operacion"),TransaccionNegocio.VENTA_TITULOS));
	}
