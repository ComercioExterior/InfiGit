	...
	*/
	...
	function constructCalendar () {
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			**/
			if(blockNext){
				if (((yearSelected > yearNext) || (monthSelected > monthNext) && (yearSelected == yearNext) || (datePointer > dayNext) && (monthSelected == monthNext) && (yearSelected == yearNext))) {
					selDayAction = '';
					isPast = 1;
				}
			}
	...
	function showCalendarBlockNext(ctl, ctl2, format, lang, past, fx, fy, day, month, year) {
		if (lang != null && lang != '') language = lang;
		if (past != null) enablePast = past;
		else enablePast = 0;
		if (fx != null) fixedX = fx;
