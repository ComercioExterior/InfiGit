...
public class TomaDeOrdenes{
	...
	private Logger logger = Logger.getLogger(TomaDeOrdenes.class);
	/**
	 ...
	 */
	public TomaDeOrdenes (DataSource dso) {
		this.dso = dso;
	...
	public ArrayList validar() throws Exception {
		...
		if(parametrosEntrada==null || parametrosEntrada.isEmpty()){
			...
			return listaMensajes;
				...
				if (montoPedido.doubleValue()== 0 && cantidadPedida == 0) {
					listaMensajes.add("Debe colocar el Monto o la Cantidad a procesar");
				}
				...
				int cantidad_minima_blot = 0;
					...
					//OJO DESCOMENTAR LINEA
					String tipoOperacion = ConstantesGenerales.BLOTTER_TIPO_OPERACION_ELEC; // EFECTIVO
				...
				if(!((String)parametrosEntrada.get("tipoTransaccionNegocio")).equals(TransaccionNegocio.TOMA_DE_ORDEN_CARTERA_PROPIA)){
					...
					//NM25287 20/04/2017 Se comenta debido a que se hacen llamadas innecesarias a los servicios
	...
	public TomaOrdenSimulada simuladorTO() throws Throwable {
		...
		try {
			if ((Long)parametrosEntrada.get("idUnidadInversion") != unidadInversionActiva){
				...
				boUISimular.listarTitulosID((Long)parametrosEntrada.get("idUnidadInversion"),false); //Modificado por NM25287. Requerimiento SICAD
...
System.out.println("TIPO PRODUCTO EN SIMULADOR SUBASTA ----"+beanTOSimulada.getTipoProductoId());
		...
		for (int i=0; i<listaTitulos.size(); i++) {			
			...
			if(!beanTitulo.getSiglasMoneda().equalsIgnoreCase(idMonedaUI)){
				...
				}else //multiplicar en caso de moneda extranjera a moneda local
	 ...
	 * @param idBlotter id del blotter 
	 * @param tipoPersona tipo de persona
	 * @return Objeto UIBlotterRangos encontrado
	 * @throws Exception en caso de no haber encontrado los rangos
	 */
	 ...
	 * @param tipo puede ser com.bdv.infi.logic.interfaces.TransaccionFija.CAPITAL_CON_IDB o com.bdv.infi.logic.interfaces.TransaccionFija.CAPITAL_SIN_IDB
	 * @throws Exception en caso de error en la búsqueda de la transaccion fija
	 */
	 ...
	 * @throws Exception
	 */
	public ArrayList validarSaldoCliente(String username, String ipTerminal, String numeroCuenta) throws IOException, JiBXException, Exception {
		ArrayList<String> ListaValidacionesSaldo = new ArrayList<String>();
		//String nombreUsuario = (String) parametrosEntrada.get("userName");
		//String ipTerminal= (String) parametrosEntrada.get("ipTerminal");
		//(String) parametrosEntrada.get("numeroCuentaCliente")
		ManejadorDeClientes mdc = new ManejadorDeClientes((ServletContext) parametrosEntrada.get("servletContext"), null);
		double saldoDisponible = 0;
		
		//Consultar saldo de la cuenta
		Cuenta cuenta=mdc.consultarSaldoCuenta(numeroCuenta, username, ipTerminal);
