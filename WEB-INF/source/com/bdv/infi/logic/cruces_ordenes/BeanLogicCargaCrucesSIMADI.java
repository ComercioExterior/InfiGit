 ...
 *
 */
public class BeanLogicCargaCrucesSIMADI {
	...
	private DataSet _resgitrosCrucesOfertas;
					...
					try {
						...
						if (celdaControl == null || tipo == HSSFCell.CELL_TYPE_BLANK) {// Se rompe la lectura de filas al encontrar la primera celda vacia (fin de registros)
							...
							if (fila == 0) {
								...
								if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV)) {
									//ITS-2912_02/12/2015 - NM26659
									plantillaCruce=true;
									...
									if(codOrdenBcvDemandaCell!=null && codOrdenBcvDemandaCell.toString().length()>0) {											
										cruce.setNroBCVOrdenDemanda(codOrdenBcvDemandaCell.toString().replaceAll("\\.0", ""));										
									} else {											
								...
								Logger.debug(this, "COD ORDEN INFI CELL------" + cruce.getIdOrdenInfiString());
								Logger.debug(this, "CEDULA CELL------" + cruce.getCiRif());								
								...
								try {									
									...
									if (validarCamposCruce((fila + 1),plantillaCruce)) { // Todos los campos son validos
										...
										/*if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV)) {
											...
											if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_AV)) {
												cruce.setEstatus(ConstantesGenerales.STATUS_NO_CRUZADA);
											}
										}*/
	...
	public boolean validarCamposCruce(int nroFila,boolean plantillaCruce) throws ParseException, Exception {
		...
		try {
			...
			if(plantillaCruce){				
				...
				if(!validCampos)++contadorError;
			} else {//NM26659 - 08/12/2015 Correccion de incidencia (Permite Cargar Registros de Cruce y No Cruce asociado a una misma Orden)
				validCampos=validacionNoCruceDemanda(nroFila, sumCrucesDemanda, cruce);
				if(!validCampos)++contadorError;
			}
	...
	private boolean validacionOrdenDemanda(int nroFila,HSSFCell codOrdenDemandaCell,HSSFCell codOrdenBcvDemandaCell) throws Exception{
		...
		if (codOrdenDemandaCell == null){
			...
			Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': El campo Codigo de Orden no puede estar vacio");
				...
				if (codOrdenDemandaCell != null && (codOrdenDemandaCell.toString() != null && codOrdenDemandaCell.toString().length() > 10)) { // SE VALIDA QUE NO EXCEDA EL TAM MAX DEL CAMPO EN BD
			...
			if (ordenExiste) {	//SI LA ORDEN EXISTE EN BASE DE DATOS						
				if(validacionBcvEnLinea){ // SI EL PARAMETRO VALIDACION BCV EN LINEA ESTA ACTIVO																						
					if(orden.getStatusVerificacionBCV().equalsIgnoreCase(ConstantesGenerales.SIN_VERIFICAR)){//ORDEN SIN VERIFICAR POR EL BCV										
						validCampos = false;				
						descripcionError += "Fila " + nroFila + " - ";
					
						if (codOrdenDemandaCell != null) {											
							descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";													
							descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': Paramatro Validacion BCV en Linea se encuentra activo y la orden no ha sido verificada por el BCV ";				
							Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': : Paramatro Validacion BCV en Linea se encutra activo y la orden no ha sido verificada por el BCV ");							
						}							
					}					
					cruce.setNroBCVOrdenDemanda(orden.getIdOrdeneBCV());				
				} else {// PARAMETRO VALIDACION BCV INACTIVO
					
					if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV)) {	//********* DANIEL *********						
						//VERIFICACION SI PROCESAMIENTO ES MANUAL Y LA ORDEN YA FUE VERIFICADA VIA BCV
						if(orden.getStatusVerificacionBCV().equalsIgnoreCase(ConstantesGenerales.VERIFICADA_APROBADA) && (cruce.getNroBCVOrdenDemanda().length()>0)){						
							validCampos = false;
							descripcionError += "Fila " + nroFila + " - ";
							if (codOrdenDemandaCell != null)
								descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
							descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_BCV_DEMANDA + "':  La orden fue verificada via BCV en linea por lo que no puede colocar colocar informacion en el campo | ";
							Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_BCV_DEMANDA + "': La orden fue verificada via BCV en linea por lo que no puede colocar colocar informacion en el campo ");
						}
						
						if(orden.getStatusVerificacionBCV().equalsIgnoreCase(ConstantesGenerales.SIN_VERIFICAR)){						
						    
							//14/12/2015 NM26659 Inclusion validacion campo Codigo BCV Demanda no se puede colocar vacio							
							if(cruce.getNroBCVOrdenDemanda() == null || cruce.getNroBCVOrdenDemanda().equals("")){
								validCampos = false;
								descripcionError += "Fila " + nroFila + " - ";
								if (codOrdenDemandaCell != null)
									descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
								descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_BCV_DEMANDA + "': Campo no puede estar vacio | ";
								Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_BCV_DEMANDA + "': Campo no puede estar vacio");
							}
							
							// VALIDACION NRO COTIZACION (FORMA)						
							if (cruce.getNroBCVOrdenDemanda() != null && cruce.getNroBCVOrdenDemanda().length() > 20) { // SE VALIDA QUE NO EXCEDA EL TAM MAX DEL CAMPO EN BD
								// Se trunca el valor a 10 caracteres
								...
								Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_BCV_DEMANDA + "': Formato invalido: No debe exceder de 20 digitos");						
							}											
				...
				//ORDEN DEMANDA 				
				if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV) && orden.getStatusVerificacionBCV().equalsIgnoreCase(ConstantesGenerales.VERIFICADA_RECHAZADA)) {
					validCampos = false;
				 ...
				 * if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_AV) && orden.getStatusVerificacionBCV().equalsIgnoreCase(ConstantesGenerales.VERIFICADA_APROBADA)) {
					validCampos = false;
	...
	private boolean validacionOrdenOferta(int nroFila,HSSFCell codOrdenOfertaCell,HSSFCell codOrdenBcvOfertaCell)throws Exception {
		...
		boolean ordenExiste=false;			
		//VALIDACION SI EL ID ES NULO
		//Codigo comentado para correccion de incidencia ITS-2960 NM26659_14/01/2016
		...
		//VALIDACION SI EL ID ES MAYOR A 10 DIGITOS				
		if (codOrdenOfertaCell != null && codOrdenOfertaCell.toString().length() > 10) { // SE VALIDA QUE NO EXCEDA EL TAM MAX DEL CAMPO EN BD
		...
		if(cruce.getIdOrdenOferta()>0){									
				...
				if(ordenExiste) {
					...
					if(validacionBcvEnLinea){ // SI EL PARAMETRO VALIDACION BCV EN LINEA ESTA ACTIVO		
						...
						//Verificacion del codigo de Orden BCV 	//Complemento de validacion incluido en correccion de incidencia ITS-2960 NM26659_14/01/2016						
						//if(!cruce.getNroBCVOrdenOferta().equals("")){
						if(cruce.getNroBCVOrdenOferta()!=null && (!cruce.getNroBCVOrdenOferta().equals("") || cruce.getNroBCVOrdenOferta().trim().length()>0)){
							validCampos = false;
							descripcionError += "Fila " + nroFila + " - ";
					...
					//ORDEN OFERTA 
					if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV) && ordenOferta.getEstatusBcvOferta().equalsIgnoreCase(ConstantesGenerales.VERIFICADA_RECHAZADA)) {
						validCampos = false;
						descripcionError +=  ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': La orden fue verificada como Rechazada por BCV y se esta cargando como CRUZADA en la aplicacion | ";
					...
					/* NM25287 13/04/2015 Se comenta para permitir el No cruce de operaciones aprobadas por BCV
					 * if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_AV) && ordenOferta.getEstatusBcvOferta().equalsIgnoreCase(ConstantesGenerales.VERIFICADA_APROBADA)) {
						validCampos = false;
						descripcionError += ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': La orden fue verificada como APROBADA por BCV y se esta cargando como NO CRUZADA en la aplicacion | ";
...
//		 VALIDACION UNIDAD DE INVERSION (CONTRA UI SELECCIONADA)
	...
	private boolean validacionFechaValor(int nroFila,HSSFCell fechaValorCell)throws Exception {
		...
		try {
			...
			boolean fValRepOferta = false;
			String codigoOferta="";
			String fechaValorOferta="";
			String dia = "", mes = "", anio = "";		
			...
			if (m.find()) { // Si el registro coincide con el formato correspondiente
				...
				if (_resgitrosCruces.count() > 0) {
					...
					//NM26659_30/04/2015 Se agrega validacion if(_resgitrosCruces.getValue("fecha_valor")!=null) para evitar error cuando existen registros de no cruce  
					if(_resgitrosCruces.getValue("fecha_valor")!=null){
				...
				//ITS-2912_02/12/2015 - NM26659 Se agrega Validacion de la fecha Valor asociado a registros cargados de la Oferta 
				if (_resgitrosCrucesOfertas.count() > 0) {					
					_resgitrosCrucesOfertas.first();
					_resgitrosCrucesOfertas.next();
					codigoOferta=_resgitrosCrucesOfertas.getValue("ordene_id_oferta");
					fechaValorOferta=_resgitrosCrucesOfertas.getValue("fecha_valor");
					//NM26659_30/04/2015 Se agrega validacion if(_resgitrosCruces.getValue("fecha_valor")!=null) para evitar error cuando existen registros de no cruce  
					if(_resgitrosCrucesOfertas.getValue("fecha_valor")!=null){
						if (com.bdv.infi.util.Utilitario.StringToDate(_resgitrosCrucesOfertas.getValue("fecha_valor"), ConstantesGenerales.FORMATO_FECHA2).compareTo(com.bdv.infi.util.Utilitario.StringToDate(cruce.getFechaValor(), ConstantesGenerales.FORMATO_FECHA2)) != 0) {
							fValRepOferta = true;
						}
					}
				}
				...
				if (fValRep) {
					...
					if (codOrdenDemandaCell != null)
						descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
					descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_FECHA_VALOR + "': El valor no se corresponde con el seteado para un cruce de Demanda asociado a la misma orden (" + cruce.getFechaValor() + ") cargado previamente | ";
					Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_FECHA_VALOR + "': El valor no se corresponde con el seteado para un cruce de Demanda asociado a la misma orden (" + cruce.getFechaValor() + ") cargado previamente |");
				}
				...
				if (fValRepOferta) {
					...
					descripcionError += "Fila " + nroFila + " - ";
					if (codOrdenDemandaCell != null)
						descripcionError += "Oferta " + codigoOferta.replaceAll("\\.0", "") + " - ";
					descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_FECHA_VALOR + "': El valor no se corresponde con el seteado para un cruce de Oferta asociado a la misma orden (" + fechaValorOferta + ") cargado previamente | ";
					Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_FECHA_VALOR + "': El valor no se corresponde con el seteado para un cruce de Oferta asociado a la misma orden (" + fechaValorOferta + ") cargado previamente | ");
				}
			} else {
				validCampos = false;
				descripcionError += "Fila " + nroFila + " - ";
	...
	private boolean validacionCruceDemanda(int nroFila,BigDecimal sumCrucesDemanda,OrdenesCruce cruce)throws Exception{
		boolean validCampos=true;
		int compare;
		BigDecimal sumCrucesDemandaBD=new BigDecimal(0);
		
		BigDecimal totalAcumuladoCrucesDemanda=new BigDecimal(0);
		if (cruce.getIdOrdenINFI() > 0) { // Se seteo el ID Orden traido desde el archivo
			...
			if (ordenesCruces.containsKey(cruce.getIdOrdenInfiString())) {
				...
				if (compare == 0) { // Ya existe un No Cruce
					...
					if (compare > 0) { // Ya existe un Cruce
						if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_AV)) { // NO CRUCE
							validCampos = false;
							descripcionError += "Fila " + nroFila + " - ";
							if (codOrdenDemandaCell != null)
								descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
							...
							if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_AV)) { // CRUCE
				...
				if (_resgitrosCruces.getValue("ESTATUS").equals(StatusOrden.NO_CRUZADA)) {
					...
					if (identificadorPlantilla.equals(ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_AV)) { // NO CRUCE
						...
						for (int i = 0; i < _resgitrosCruces.count(); i++) {
							...
							sumCrucesDemandaBD = sumCrucesDemandaBD.add(new BigDecimal(_resgitrosCruces.getValue("MONTO_OPERACION")));
						}
						sumCrucesDemandaBD = sumCrucesDemandaBD.setScale(2, BigDecimal.ROUND_HALF_UP);
			...
			totalAcumuladoCrucesDemanda=sumCrucesDemanda.add(sumCrucesDemandaBD);
			...
			BigDecimal mtoDisponible=new BigDecimal(orden.getMonto()).subtract(totalAcumuladoCrucesDemanda);
	 ...
	 * @author NM26659
	 */
	//ITS-2912_02/12/2015 - NM26659 Metodo de Validacion de registros de NO CRUCE
	private boolean validacionNoCruceDemanda(int nroFila,BigDecimal sumCrucesDemanda,OrdenesCruce cruce)throws Exception{
		boolean validCampos=true;
		int compare;
		...
		if (cruce.getIdOrdenINFI() > 0) { // Se seteo el ID Orden traido desde el archivo			
			...
			if (ordenesCruces.containsKey(cruce.getIdOrdenInfiString())) {
				...
				if (compare == 0) { // Ya existe un No Cruce
					...
					if (codOrdenDemandaCell != null)
						descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
					descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': No puede realizar un NO CRUCE a la orden puesto que ya existe un registro de no cruce por cargar asociado a la misma | ";
					Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': No puede realizar un cruce/no cruce a la orden puesto que ya existe un registro de no cruce por cargar asociado a la misma");
				}
			}// Ya existen cruces/no cruces en el lote de carga para la orden
			...
			_resgitrosCruces = ordenesCrucesDAO.getDataSet();
			if (_resgitrosCruces.count() > 0) {		
				//NM26659 27/11/2015 Correccion de Incidencia Carga de Operaciones No Cruzadas con registros 				
					validCampos = false;
					descripcionError += "Fila " + nroFila + " - ";
					if (codOrdenDemandaCell != null)
						descripcionError += "Orden " + codOrdenDemandaCell.toString().replaceAll("\\.0", "") + " - ";
					descripcionError += "Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': No puede realizar un NO CRUCE a la orden puesto que ya existe un registro cargado | ";
					Logger.info(this, "Error al procesar archivo de cruces '" + getNombreDocumento() + "' en la Fila " + nroFila + " - Campo '" + ConstantesGenerales.CAMPO_PLANTILLA_COD_ORDEN_DEMANDA + "': No puede realizar un no cruce a la orden puesto que ya existe un registro de cruce asociado a la misma");				
				//NM26659 27/11/2015 Correccion de Incidencia Carga de Operaciones No Cruzadas con registros               	
			}		
		}		
		return validCampos;
	...
	private boolean validacionCruceOferta(int nroFila,BigDecimal sumCrucesOferta,OrdenesCruce cruce)throws Exception{
		...
		if (cruce.getIdOrdenOferta() > 0) { // Se seteo el ID Orden traido desde el archivo		
			...
			_resgitrosCrucesOfertas = ordenesCrucesDAO.getDataSet();
			if (_resgitrosCrucesOfertas.count() > 0) {
				_resgitrosCrucesOfertas.first();
				_resgitrosCrucesOfertas.next();				
				 // ES UN CRUCE
				...
				_resgitrosCrucesOfertas.first();
						// for(int i=0; i<ordenesCrucesDAO.getDataSet().count(); i++){
						for (int i = 0; i < _resgitrosCrucesOfertas.count(); i++) {
							// ordenesCrucesDAO.getDataSet().next();
							_resgitrosCrucesOfertas.next();
							// sumCrucesDemanda = sumCrucesDemanda.add(new BigDecimal(ordenesCrucesDAO.getDataSet().getValue("MONTO_OPERACION")));
							sumCrucesOferta = sumCrucesOferta.add(new BigDecimal(_resgitrosCrucesOfertas.getValue("MONTO_OPERACION")));						
						}
