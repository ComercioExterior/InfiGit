...
public class AbonoCuentaNacionalMonedaExtranjeraPagoCupones extends CuentaNacionalMonedaExtranjeraPagoCupones implements Runnable{

	
	Logger logger = Logger.getLogger(AbonoCuentaNacionalMonedaExtranjera.class);
	
	public AbonoCuentaNacionalMonedaExtranjeraPagoCupones(DataSource datasource, int usuarioId,
			String nmUsuario) {
	...
	public void run() {
		...
		logger.info(" ************ Iniciando el proceso de generación de archivo batch para Abono en cuenta nacional de moneda Extranjera tipo PAGO CUPÓN ************");
		
		try {
			
			if (verificarCiclo("'"+TransaccionNegocio.CICLO_BATCH_CTA_NACIONAL_MONEDA_EXT_PAGO_CUPON+"'", "") &&								
					comenzarProceso(TransaccionNegocio.PROC_BATCH_CTA_NAC_MON_EXT_CUPON_ENVIO)) {	
				...
				logger.info("Existe un ciclo de abono cuenta nacional en dolares para Pago Cupón Activo o Error en registro de proceso de Abono en moneda extranjera Venta Título");
			}
			
		} catch (Exception ex){
			logger.error(" Error en el proceso de generacion de archivo de abono cuenta nacional en moneda extranjera Pago Cupón -----> " + ex.getMessage());
			if(proceso != null) {
				proceso.setDescripcionError(ex.getMessage());
			}
		} finally {			
			terminarProceso();
			logger.info("Terminado el proceso de generación de archivo batch para Abono cuenta nacional moneda extranjera Pago Cupón... ");
		}
	}
	...
	protected String getRegistrosAProcesar() throws Exception{
		...
		return ordenDAO.detalleDeAbonoCuentaDolaresPagoCupones();		
	}
...
//        return new File(archivoFinal);
	...
	protected String getCiclo(){
		return TransaccionNegocio.CICLO_BATCH_CTA_NACIONAL_MONEDA_EXT_PAGO_CUPON;
	}
	
	protected String getNombreArchivo(){
		return ParametrosSistema.NOMBRE_ARCH_CUPON_MONEDA_EXT;
	}
	...
	protected void obtenerArchivoTemporal(){
		//Directorio temp para guardar los archivos
        archivoTemporal = parametrosOPS.get(ParametrosSistema.RUTA_CUPON_MONEDA_EXT_ENVIO);
	}
