...
public class AbonoCuentaNacionalMonedaExtranjera extends CuentaNacionalMonedaExtranjera implements Runnable{

	
	Logger logger = Logger.getLogger(AbonoCuentaNacionalMonedaExtranjera.class);
	private String idOrdenesSeleccionadas;
	public AbonoCuentaNacionalMonedaExtranjera(DataSource datasource, int usuarioId,
			String nmUsuario, int idUnidadInversion,String idOrdenes) {
	...
	public void run() {
		...
		logger.info(" ************ Iniciando el proceso de generación de archivo batch para Abono en cuenta nacional de moneda Extranjera tipo SUBASTA DIVSAS CLAVENET PERSONAL ************");
		
		try {
			
			if (verificarCiclo("'"+TransaccionNegocio.CICLO_BATCH_CTA_NACIONAL_MONEDA_EXT_SUB_DIV_P+"'", "") &&//TransaccionNegocio.CICLO_BATCH_CTA_NACIONAL_MONEDA_EXT_SITME								
					comenzarProceso(TransaccionNegocio.PROC_BATCH_CTA_NAC_MON_EXT_SUB_DIV_P_ENVIO)){//TransaccionNegocio.PROC_BATCH_CTA_NACIONAL_MONEDA_EXT_SITME_ENVIO 									
				obtenerParametros();						
				final File archivo = getArchivoEnvioMonedaExtranjeraSitme();
						
				//verificarArchivoFinal(archivo);					
				envioArchivo(ConstantesGenerales.OPS_MONEDA_EXTRANJERA);			
				respaldarArchivo(archivo,true);
			} else {
				//System.out.println("********** Existe un ciclo de abono cuenta nacional en dolares Activo o Error en registro de proceso de Abono en moneda extranjera ***********");
				logger.info("Existe un ciclo de abono cuenta nacional en dolares Activo o Error en registro de proceso de Abono en moneda extranjera");
			}
			
		} catch (Exception ex){
			logger.error(" Error en el proceso de generacion de archivo de abono cuenta nacional en moneda extranjera -------> " + ex.getMessage());
			if(proceso != null) {
				proceso.setDescripcionError(ex.getMessage());
			}
		} finally {			
			terminarProceso();
			logger.info("Terminado el proceso de generación de archivo batch para Abono cuenta nacional moneda extranjera... ");
		}
	}
	...
	protected String getRegistrosAProcesar() throws Exception{
		...
		return inversionDAO.detalleDeAbonoCuentaDolares(idUnidadInversion,idOrdenesSeleccionadas,false);		
	}
...
//        return new File(archivoFinal);
	...
	protected String getCiclo(){
		return TransaccionNegocio.CICLO_BATCH_CTA_NACIONAL_MONEDA_EXT_SUB_DIV_P;
	}
	
	protected String getNombreArchivo(){
		return ParametrosSistema.NOMBRE_ARCH_SITME_MONEDA_EXT;
	}
	...
	protected void obtenerArchivoTemporal(){
		//Directorio temp para guardar los archivos
        archivoTemporal = parametrosOPS.get(ParametrosSistema.RUTA_SITME_MONEDA_EXT_ENVIO);
	}
