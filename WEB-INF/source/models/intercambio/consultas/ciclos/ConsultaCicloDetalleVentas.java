...
public class ConsultaCicloDetalleVentas extends ExportableOutputStream {
	...
	public void execute() throws Exception {
		...
		try {
			...
			logger.error("Error en el proceso de generación archivo batch para Abono Cuenta en Dolares Moneda Extranjera Ops de Ventas. " + ex.getMessage(), ex);
		} finally {
			try {
				if (operaciones != null) {
					operaciones.close();
	...
	protected void getOperaciones() throws Exception{
		operaciones = statement.executeQuery(controlArchivoDAO.listarDetalleDelCicloVentas(Integer.parseInt(_record.getValue("idEjecucion")),getCiclo()));
	}
	...
	protected Logger getLogger(){
		return Logger.getLogger(ConsultaCicloDetalleVentas.class);
	}
}
