	...
	public void execute() throws Exception {
		...
		if(seleccion.equalsIgnoreCase("todos")){			
			...
			if(tipoFiltro.equals("EXCLUIR")){
				...
				listaTodos = getSessionDataSet("listarUnidadesParaCobroBatchOfertaDicom");
				boolean flag = true;
...
/*		statusUnidadInv.add(UnidadInversionConstantes.UISTATUS_PUBLICADA);
		...
		*/
		...
		setSessionDataSet("listarUnidadesParaCobroBatchOfertaDicom", null);
	}
	...
	public boolean isValid() throws Exception {
		...
		if (procesosDAO.getDataSet().count() > 0) {
			...
			if (dataSet.count() > 0){
				...
				if (procesosDAO.getDataSet().count() > 0) {
					_record.addError("Cobro Operaciones DICOM",
