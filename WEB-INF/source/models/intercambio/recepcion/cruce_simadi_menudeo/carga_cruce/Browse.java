package models.intercambio.recepcion.cruce_simadi_menudeo.carga_cruce;
...
import com.bdv.infi.logic.cruces_ordenes.BeanLogicCargaCrucesMenudeo;
...
public class Browse extends MSCModelExtend{
	...
	if(record == null){
		...
		if(unidadInversionDAO.getDataSet().count()>0){
			...
			tipoProducto=unidadInversionDAO.getDataSet().getValue("tipo_producto_id");
			
			if(tipoProducto.equals(ConstantesGenerales.ID_TIPO_PRODUCTO_SICADII_CLAVENET_PERSONAL)){
				transaccionId = TransaccionNegocio.CRUCE_SIMADI_MENUDEO_NATURAL_CARGA;				
			}else{
				if(tipoProducto.equals(ConstantesGenerales.ID_TIPO_PRODUCTO_SICADII_RED_COMERCIAL)){
					transaccionId = TransaccionNegocio.CRUCE_SIMADI_MENUDEO_JURIDICO_CARGA;					
				}
			}
		}
		...
		BeanLogicCargaCrucesMenudeo beanLogicCargaCruces = new BeanLogicCargaCrucesMenudeo();		
		...
		if(flag)
		{
			...
			if(flag){
				...
				if(_record.getValue("tipo_operacion").equals(ConstantesGenerales.INDICADOR_CRUCE)){
					System.out.println("INDICADOR CRUCE ---> " + _record.getValue("tipo_operacion"));
					plantillaTitulo = ConstantesGenerales.TITULO_TEMPLATE_CRUCE_SIMADI_MENUDEO;
					plantillaName = ConstantesGenerales.ARCH_TEMPLATE_CRUCE_SIMADI_MENUDEO;
					...
					if(_record.getValue("tipo_operacion").equals(ConstantesGenerales.INDICADOR_NO_CRUCE)){
						System.out.println("INDICADOR NO CRUCE ---> " + _record.getValue("tipo_operacion"));
						plantillaTitulo = ConstantesGenerales.TITULO_TEMPLATE_NO_CRUCE_SIMADI_MENUDEO;
						plantillaName = ConstantesGenerales.ARCH_TEMPLATE_NO_CRUCE_SIMADI_MENUDEO;
						if(!_record.getValue("archivo.filename").startsWith(plantillaName)){//CT19940 - Flexibilizacion de la validacion del nombre del archivo de carga.
							_record.addError("Archivo", "El nombre del archivo que ingres&oacute; ("+_record.getValue("archivo.filename")+") no coincide con el nombre de la plantilla correspondiente a la operaci&oacute;n seleccionada ("+plantillaName+"). Verifique e intente de nuevo");
							flag = false;
						}
