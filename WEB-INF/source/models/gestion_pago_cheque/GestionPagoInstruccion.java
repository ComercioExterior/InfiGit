...
/**
 * Clase que se encarga de la toma de datos para las instrucciones de cheque y transferencias
 *@author elaucho
 */
public class GestionPagoInstruccion extends MSCModelExtend{

	@Override
public void execute() throws Exception {

		//Verificamos si hay control cambiario
...
//Instruccion pago NACIONAL moneda Extranjera (Operacion Cambio)												 
	}else if(seleccion.equals("nacionales") && !monedaSesion.equalsIgnoreCase(monedaLocal)){
 ...
 */
	BigDecimal montoOperaciones = _req.getSession().getAttribute("infi.monto.operaciones")!=null?new BigDecimal(_req.getSession().getAttribute("infi.monto.operaciones").toString()):new BigDecimal(0);
 ...
 */
	/*if(operacionCambio){
		...
		try {
			...
			//String tasaVenta = tcmccc1.getVenta();				
			tasaVenta = tcmccc1.getVenta().toString();			
		} catch (Exception e) {			
			e.printStackTrace();
			throw new Exception("Error buscando la tasa de cambio de la moneda.");
		}
		...
		_operacionesmonto.setValue("operaciones_monto",Utilitario.formatearNumero(String.valueOf(montoCambio), "###,###,###,###.####"));
		...
		_operacionesmonto.setValue("operaciones_monto",Utilitario.formatearNumero(String.valueOf(montoOperaciones), "###,###,###,###.####"));
		_req.getSession().setAttribute("infi.monto.operaciones",montoOperaciones.setScale(2, BigDecimal.ROUND_HALF_EVEN));
		...
		_operacionesmonto.setValue("monto_operaciones_sin_formato",_req.getSession().getAttribute("infi.monto.operaciones").toString());
	}
