...
HTMLHttpRequest.prototype.iframeCheck = function() { with (this)
{
 ...
 if ((il && il.href ? il.href.match(  encodeRE(loadingURI) ) : 1) &&
     (dr == 'complete' || (!document.getElementById && dr == 'interactive')))
 {
  if (callback) callback((doc.documentElement || doc), loadingURI);
  loadingURI = '';
 }
 else setTimeout(myName + '.iframeCheck()', 80);
}};

HTMLHttpRequest.prototype.encodeRE = function (s) { return s.replace(/([.*+?^${}()|[\]\/\\])/g, '\\$1') } 
